    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        if isinstance(inputs, (list,)):
            for i in range(len(inputs)):
                ...
                inputs[i] = inputs[i] if not fp16 else inputs[i].half()
        else:
            inputs = inputs.cuda(non_blocking=True)
            inputs = inputs if not fp16 else inputs.half()
        ...
        train_meter.update_stats(top1_err, top5_err, loss, lr,
            inputs[0].size(0) * max(dist.get_world_size(), 1),  
...
@torch.no_grad()
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, is_dist=False, writer=None, fp16=False):
    """
...
def build_trainer(args, cfg):
    ...
    train_loader = loader.construct_loader(cfg, "train", args.distributed)
    val_loader = loader.construct_loader(cfg, "val", args.distributed)
    precise_bn_loader = loader.construct_loader(
        cfg, "train", is_precise_bn=True, is_dist=args.distributed
    )
    # Create meters.
...
def train(args, cfg):
    ...
    if cfg.TRAIN.CHECKPOINT_FILE_PATH != "":
        logger.info(f"Loading model weights from {cfg.TRAIN.CHECKPOINT_FILE_PATH}")
        start_epoch = cu.load_train_checkpoint(cfg, model, optimizer)
    if args.resume:
        start_epoch = cu.resume_checkpoint(model_engine, args.blob_mount_dir, cfg.OUTPUT_DIR)
        
    if du.is_master_proc(args.distributed) and cfg.LOG_MODEL_INFO:
        misc.log_model_info(model, cfg, use_train_input=True, fp16=args.fp16)
    ...
    # Create the video train and val loaders.
    train_loader = loader.construct_loader(cfg, "train", is_dist=args.distributed)
    val_loader = loader.construct_loader(cfg, "val", is_dist=args.distributed)
    precise_bn_loader = (
        loader.construct_loader(cfg, "train", is_precise_bn=True, is_dist=args.distributed)
        if cfg.BN.USE_PRECISE_STATS else None
    )
    
    # Create scheduler
    #steps_per_epoch = len(train_loader)
    #scheduler = optim.construct_scheduler(cfg, optimizer, steps_per_epoch)
    ...
    if cfg.TENSORBOARD.ENABLE and du.is_master_proc(args.distributed):
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        if cfg.MULTIGRID.LONG_CYCLE:
            ...
            if changed:
                model_engine, optimizer, train_loader, val_loader, \
                    precise_bn_loader, train_meter, val_meter = build_trainer(args, cfg)
                ...
                cu.load_checkpoint(
                    last_checkpoint, model_engine, cfg.NUM_GPUS > 1, optimizer
                )
        ...
        #eval_epoch(val_loader, model_engine, val_meter, cur_epoch, cfg, args.distributed, writer, args.fp16)
        train_epoch(train_loader, model_engine, optimizer, train_meter, 
                    cur_epoch, cfg, args.distributed, writer, args.fp16)
        ...
        if (
            ...
            and len(get_bn_modules(model_engine)) > 0
        ):
        ...
        _ = misc.aggregate_sub_bn_stats(model_engine)

        # Save a checkpoint.
        if is_checkp_epoch:
            cu.save_checkpoint(model_engine, cur_epoch, args.blob_mount_dir, cfg.OUTPUT_DIR, args.fp16)
